...
export default function Navbar() {
    ...
    return (
        <>
            ...
            >
                <Link to='/'>
                    <img src="/logo.svg" alt="Logo" className="w-35 h-35 object-contain" />
                </Link>


                <div className="hidden md:flex items-center gap-8 transition duration-500">
                    <Link to='/' className="active:text-pink-500 transition"> Home </Link>
                    <Link to='/generate' className="active:text-pink-500 transition"> Generate </Link>
                    <Link to='/my-generations' className="active:text-pink-500 transition">  My Generations </Link>
                    <Link to='#' className="active:text-pink-500 transition">  My Contact </Link>
            ...
            <div className={`fixed inset-0 z-100 bg-black/40 backdrop-blur flex flex-col items-center justify-center text-lg gap-8 md:hidden transition-transform duration-400 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
                ...
                <Link onClick={() => setIsOpen(false)} to="/login" className="px-6 py-2.5 bg-pink-600 hover:bg-pink-700 active:scale-95 transition-all rounded-full">Login</Link>
